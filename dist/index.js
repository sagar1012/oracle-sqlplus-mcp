...
if (!CONNECTION_STRING) {
    process.stderr.write("[oracle-mcp] ERROR: ORACLE_CONNECTION env var is required.\n" +
...
function formatQueryResult(result, label) {
    if (!result.success) {
...
const TOOLS = [
    {
        name: "execute_query",
        description: "Execute a SQL SELECT query against the Oracle database and return the results. " +
    ...
    {
        name: "execute_ddl",
        description: "Execute a DDL or DML statement (CREATE, INSERT, UPDATE, DELETE, ALTER, DROP). " +
    ...
    {
        name: "list_tables",
        description: "List all tables accessible to the current user, optionally filtered by schema/owner.",
        inputSchema: {
            type: "object",
            properties: {
                schema: {
                    type: "string",
                    description: "Optional schema/owner name to filter tables (e.g. SCOTT). " +
    ...
    {
        name: "list_schemas",
        description: "List all schemas (users/owners) in the database that have at least one table.",
        inputSchema: {
            type: "object",
            properties: {},
        },
    ...
    switch (name) {
        ...
        case "list_tables": {
            ...
            if (schema) {
                ...
                if (search)
                    sql += ` WHERE TABLE_NAME LIKE '${search}'`;
                sql += " ORDER BY TABLE_NAME";
            }
        ...
        case "describe_table": {
            ...
            const ownerClause = owner === "USER" ? "OWNER = USER" : `OWNER = '${owner}'`;
            const sql = `
SELECT
  COLUMN_NAME,
  DATA_TYPE ||
...
ORDER BY COLUMN_ID`;
            const result = await runSqlPlus(sql);
            if (!result.success)
                return formatQueryResult(result, "Describe Table");
...
// ─── Start ────────────────────────────────────────────────────────────────────
async function main() {
    process.stderr.write(`[oracle-mcp] Starting Oracle MCP Server (sqlplus mode)\n` +
